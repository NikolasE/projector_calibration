...
bool Projector_Calibrator::eval_projection_matrix_disc(Cloud& mean_c, std::stringstream& ss, const cv::Mat* area_mask){
  ...
  */
   ...
   //  ROS_INFO("%i: area: %f",i,area);
    ...
    if (area < min_area)
      ...
      if (area > largest_area){
       // ROS_INFO("new largest: %i %f",i,area);
        largest_area = area;
        largest_contour = i;
      }
    }
  ...
  //ROS_INFO("largest contour (%i) area: %f",largest_contour,largest_area);


  cv::drawContours(bright, contours, largest_contour, cv::Scalar::all(125), 2, 8, hierarchy, 0, cv::Point());
...
bool Projector_Calibrator::storeCurrentObservationPairs(){
  ...
  // transform from kinect-frame to world-frame
